...
def calc_flops(args):
    model_handler = ModelHandler(args.model_path, args.threshold)
    input_tensor = torch.randn(1, 24000)

    with torchprofile.profile(model_handler, input_tensor=input_tensor) as prof:
        model_handler(input_tensor)
        flops = prof.total()
    return flops
...
def main(args):
        ...
        print(f"The DCF for the threshold {args.threshold} is {dcf}")
        start_time = time.time()
        ...
        print(f"The DCF for the threshold {args.threshold} in extended test is {dcf_ext}")
        #Mflops = calc_flops(args) / 1e6
        row = [args.model_name, args.hop, args.time_window, args.threshold, args.n_positives , dcf, dcf_ext, pmiss,  pmiss_ext, p_fa, p_fa_ext, tem, time_test, time_extended]
        with open("Results_eval.tsv", "a", newline="") as tsv:
